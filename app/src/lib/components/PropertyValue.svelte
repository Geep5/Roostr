<script lang="ts">
	...
	const HIDDEN_TYPES: Record<string, true> = { program: true, relation: true, channel: true, pinned_fact: true, milestone: true, type: true, template: true };
...
{:else if rel.format === "object"}
	<div class="objects">
		{#each items as id (id)}
			...
			<span class="obj-chip">
				<a href="/object/{id}">{objectIcon(undefined, s?.typeKey ?? "note")} {nameOf(id)}</a>
