...
//   change id = sha256(encode(change with the zeroed id tag 0a00))
...
Block :: struct {
	...
	has_content:      bool, // preserve explicitly present empty messages
	has_fields:       bool,
}
...
Operation :: struct {
	...
	omit_payload: bool, // missing optional value/block/content differs from {}
}
...
read_bytes :: proc(r: ^Reader) -> []byte {
	size := read_varint(r)
	if r.err || size > u64(len(r.data) - r.pos) {
		r.err = true
		return nil
	}
	n := int(size)
...
decode_change :: proc(data: []byte, allocator := context.allocator) -> (Change, bool) {
	context.allocator = allocator
	if !valid_message(data, .Change) do return {}, false
	r := Reader{data = data}
	c: Change
	for r.pos < len(r.data) && !r.err {
		tag := read_varint(&r)
...
encode_value :: proc(v: Value, w: ^Writer) {
	...
	case .Map:
		...
		for e in v.entries {
			entry: Writer
			write_len_prefixed(&entry, 1, transmute([]byte)e.key)
			vw: Writer
...
encode_block_content :: proc(c: Block_Content, w: ^Writer) {
	...
	case .Custom:
		...
		for p in c.custom.meta {
			pw: Writer
			write_len_prefixed(&pw, 1, transmute([]byte)p.key)
			write_len_prefixed(&pw, 2, transmute([]byte)p.value)
...
encode_block :: proc(b: Block, w: ^Writer) {
	...
	if b.has_content || b.content.kind != .None {
		cw: Writer
		encode_block_content(b.content, &cw)
		write_len_prefixed(w, 3, cw.buf[:])
		delete(cw.buf)
	}
	if b.has_fields || len(b.fields) > 0 {
		fw: Writer
		for e in b.fields {
			entry: Writer
			write_len_prefixed(&entry, 1, transmute([]byte)e.key)
...
/** Encode a change; hashing emits the legacy zeroed id field (0a00). */
encode_change :: proc(c: Change, for_hashing := false, allocator := context.allocator) -> []byte {
	...
	if c.has_snapshot {
		sw: Writer
		encode_snapshot(c.snapshot, &sw)
		write_len_prefixed(&w, 7, sw.buf[:])
		delete(sw.buf)
	}
	return w.buf[:]
}
...
encode_snapshot :: proc(s: Snapshot, w: ^Writer) {
	write_string_field(w, 1, s.id)
	write_string_field(w, 2, s.type_key)
	for e in s.fields {
		entry, vw: Writer
		write_len_prefixed(&entry, 1, transmute([]byte)e.key)
		encode_value(e.value, &vw)
		write_len_prefixed(&entry, 2, vw.buf[:])
		write_len_prefixed(w, 3, entry.buf[:])
		delete(vw.buf)
		delete(entry.buf)
	}
	if len(s.content) > 0 do write_len_prefixed(w, 4, s.content)
	for b in s.blocks {
		bw: Writer
		encode_block(b, &bw)
		write_len_prefixed(w, 5, bw.buf[:])
		delete(bw.buf)
	}
	write_bool_field(w, 6, s.deleted)
	write_i64_field(w, 7, s.created_at)
...
wire_field :: proc(message: Wire_Message, field: u64) -> (wire: u64, nested: Wire_Message, known: bool) {
	wire, nested, known = 2, .Scalar, true
	switch message {
	case .Change:
	...
	case .Operation:
		...
		case 2: nested = .Empty
		case 3: nested = .Field_Set
		case 4: nested = .Field_Delete
		case 6: nested = .Block_Add
		case 7: nested = .Block_Remove
		case 8: nested = .Block_Update
		case 9: nested = .Block_Move
		case 10: nested = .Align
		case 11: nested = .Background
		case: known = false
		}
	case .Snapshot:
		switch field { case 1, 2, 4: case 3: nested = .Value_Entry; case 5: nested = .Block; case 6, 7, 8: wire = 0; case: known = false }
	case .Value:
		switch field { case 1, 5: case 2, 4: wire = 0; case 3: wire = 1; case 6: nested = .String_List; case 7: nested = .Value_Map; case 8: nested = .Value_List; case 9: nested = .Link; case: known = false }
	case .Value_Map:
		known, nested = field == 1, .Value_Entry
	case .Value_Entry:
		switch field { case 1: case 2: nested = .Value; case: known = false }
	case .Value_List:
		known, nested = field == 1, .Value
	case .String_List, .Create, .Field_Delete, .Block_Remove:
		known = field == 1
	case .Link, .String_Entry, .Background:
		known = field == 1 || field == 2
	case .Block:
		switch field { case 1, 2, 6: case 3: nested = .Content; case 4: nested = .Value_Map; case 5: wire = 0; case: known = false }
	case .Content:
		switch field { case 1: nested = .Text; case 2: nested = .Custom; case 3: nested = .Layout; case 4, 5: nested = .Empty; case 6: nested = .Row; case: known = false }
	case .Text:
		switch field { case 1, 5: case 2, 4: wire = 0; case 3: nested = .Mark; case: known = false }
	case .Mark:
		switch field { case 1, 2, 3: wire = 0; case 4: case: known = false }
	case .Custom:
		switch field { case 1, 2: case 3: nested = .String_Entry; case: known = false }
	case .Layout, .Row:
		known, wire = field == 1, 0
	case .Field_Set:
		switch field { case 1: case 2: nested = .Value; case: known = false }
	case .Block_Add:
		switch field { case 1, 2, 4: case 3: nested = .Block; case 5: wire = 0; case: known = false }
	case .Block_Update:
		switch field { case 1: case 2: nested = .Content; case: known = false }
	case .Block_Move:
		switch field { case 1, 2, 3, 4: case 5: wire = 0; case: known = false }
	case .Align:
		switch field { case 1: case 2: wire = 0; case: known = false }
	case .Scalar, .Empty:
		known = false
	}
	return
...
valid_message :: proc(data: []byte, message: Wire_Message, depth: int = 0) -> bool {
	...
	for r.pos < len(r.data) && !r.err {
		...
		if known && nested != .Scalar {
			child := read_bytes(&r)
			if r.err || !valid_message(child, nested, depth + 1) do return false
		} else {
			skip_field(&r, wire)
		}
	}
